...
vector<int> work(int x)
{
    ...
    while (true)
    {
        ...
        if (get(z, y) <= 100)
        {
            ...
            while (true)
            {
                rep(i, 1, z / y) res.emplace_back(2);
                if (z % y == 0)
                    break;
                res.emplace_back(3);
                tie(z, y) = make_pair(y, z % y);
            }
            return res;
        }
    }
...
// idk whats underneath
/*
#pragma GCC optimize("Ofast")
#include<bits/stdc++.h>
using namespace std;
#define rep(Ii,Jj,Kk) for(int Ii=(Jj),Ii##=(Kk);Ii<=Ii##;Ii++)
#define per(Ii,Jj,Kk) for(int Ii=(Jj),Ii##=(Kk);Ii>=Ii##;Ii--)
typedef long long ll;
typedef unsigned long long ull;
typedef unsigned uint;
typedef long double db;
...
/*
// TBH, IDK what's going on here XD
...
#ifdef LOCAL
#define dbg(...) loc_info(_LINE, #VA_ARGS), dbg_out(VA_ARGS_)
#define dbgl(lev, x) loc_info(_LINE_, #x), dbgl_out<lev>(x)
...
mi neg_odd()
{
    ...
    F0R(i2, N)
    {
        vi all_y;
        F0R(y, i2)
        if (is_neg(A.at(y)) == is_neg(A.at(i2))) all_y.pb(y);
